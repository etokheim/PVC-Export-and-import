...
Outputs:
- `database@default.tar.gz`
- `cache@default.tar.gz`
- `redis@default.tar.gz`
...
./pv-import.sh ./my-pvc@default.tar.gz
...
./pv-import.sh ./my-pvc@default.tar
```
...
```bash
./pv-import.sh ./unifi@default.tar.gz
```
...
For example:
- PVC `unifi` in namespace `default` → `unifi@default.tar.gz`
- PVC `database` in namespace `production` → `database@production.tar.gz`

The output filename format `pvc@namespace` matches the input syntax, making it easy to identify and re-import backups.
